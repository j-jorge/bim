...
struct bim::axmol::app::online_game::fence
{
  std::uint8_t x;
  std::uint8_t y;
  bim::game::cell_edge edge;
};
  ...
  , m_arena_width_in_blocks(*style.get_number("arena-width-in-blocks"))
{
  ...
  ::alloc_assets(
      m_static_walls[(std::size_t)bim::game::cell_neighborhood::none],
      widget_context, inner_width * inner_height,
      *style.get_declaration("static-wall"), *m_controls->arena);
  ::alloc_assets(
      m_static_walls[(std::size_t)(bim::game::cell_neighborhood::all
                                   & ~bim::game::cell_neighborhood::down)],
      widget_context, inner_width, *style.get_declaration("arena-border.top"),
      *m_controls->arena);
  ::alloc_assets(
      m_static_walls[(std::size_t)(bim::game::cell_neighborhood::all
                                   & ~bim::game::cell_neighborhood::up)],
      widget_context, inner_width,
      *style.get_declaration("arena-border.bottom"), *m_controls->arena);
  ::alloc_assets(
      m_static_walls[(std::size_t)(bim::game::cell_neighborhood::all
  ...
  ::alloc_assets(
      ...
      widget_context, inner_height,
      *style.get_declaration("arena-border.right"), *m_controls->arena);
  ::alloc_assets(
      m_static_walls[(
      ...
      *m_controls->arena);
  ::alloc_assets(m_static_walls[(
                     std::size_t)(bim::game::cell_neighborhood::all
                                  & ~bim::game::cell_neighborhood::down_left)],
                 widget_context, 1,
                 *style.get_declaration("arena-border.top-right"),
                 *m_controls->arena);
  ::alloc_assets(
      m_static_walls[(std::size_t)(bim::game::cell_neighborhood::all
  ...
  constexpr int max_fence_count =
      inner_width * inner_height / bim::game::g_fence_count_ratio;
  ::alloc_assets(m_fence_assets[bim::game::cell_edge::left], widget_context,
                 max_fence_count, *style.get_declaration("fence.left"),
                 *m_controls->arena);
  ::alloc_assets(m_fence_assets[bim::game::cell_edge::right], widget_context,
                 max_fence_count, *style.get_declaration("fence.right"),
                 *m_controls->arena);
  ::alloc_assets(m_fence_assets[bim::game::cell_edge::up], widget_context,
                 max_fence_count, *style.get_declaration("fence.up"),
                 *m_controls->arena);
  ::alloc_assets(m_fence_assets[bim::game::cell_edge::down], widget_context,
                 max_fence_count, *style.get_declaration("fence.down"),
                 *m_controls->arena);
  ...
  ::alloc_assets(m_falling_blocks_shadows, widget_context, free_cell_count,
                 ...
                 *m_controls->arena);
  ::alloc_assets(m_animation_sprites, widget_context, 2 * free_cell_count,
                 *style.get_declaration("animation"), *m_controls->arena);
  ...
  ::alloc_assets(m_shield_power_ups, widget_context,
                 ...
                 *m_controls->arena);
  ::alloc_assets(m_walls, widget_context, free_cell_count,
...
void bim::axmol::app::online_game::attached()
{
  ...
  for (const std::vector<ax::Sprite*>& walls : m_static_walls)
  ...
  resize_to_block_width(m_animation_sprites);
  ...
  for (int i = 0; i != bim::game::cell_edge_count; ++i)
    resize_to_block_width(m_fence_assets[(bim::game::cell_edge)(1 << i)]);
    ...
    const bim::net::game_launch_event& event)
{
...

  {
    const bim::game::crate_animations& animations =
        m_contest->context().get<const bim::game::crate_animations>();
    m_animations[animations.idle] = &m_animation_cache->get("crate.idle");
    m_animations[animations.burn] = &m_animation_cache->get("crate.burn");
  }

  {
    const bim::game::smoke_animations& animations =
        m_contest->context().get<const bim::game::smoke_animations>();
    m_animations[animations.in_out] = &m_animation_cache->get("smoke.in_out");
  }
  ...
  m_fences.clear();
  for (std::uint8_t y = 0, h = arena.height(); y != h; ++y)
    for (std::uint8_t x = 0, w = arena.width(); x != w; ++x)
      {
        const bim::game::cell_edge e = arena.fences(x, y);
        if (e != bim::game::cell_edge::none)
          m_fences.emplace_back(x, y, e);
      }
  ...
  for (int i = 0; i != bim::game::cell_edge_count; ++i)
    hide_all(m_fence_assets[(bim::game::cell_edge)(1 << i)]);

  m_fog->displaying(m_local_player_index);

  reset_z_order();
  m_animation_sprite_index = 0;

  display_animations<bim::game::layer_zero>();
  display_players();
...
void bim::axmol::app::online_game::refresh_display()
{
  ...
  reset_z_order();
  display_fences(bim::game::cell_edge::all & ~bim::game::cell_edge::down);
  display_animations<bim::game::layer_zero>();
  display_bombs();
  display_flames();
  ...
  display_players();
  display_animations<bim::game::layer_front>();
  display_fences(bim::game::cell_edge::down);
  ...
  bim::axmol::widget::hide_while_visible(m_animation_sprites,
...
void bim::axmol::app::online_game::display_static_walls()
{
  ...
  for (const bim::game::static_wall& wall : arena.static_walls())
    {
      ...
      if (i == m_static_walls[n].size())
        continue;

      display_at(wall.y, *m_static_walls[n][i],
  ...
  for (std::size_t i = 0; i != asset_index.size(); ++i)
    bim::axmol::widget::hide_while_visible(m_static_walls[i], asset_index[i]);
}

void bim::axmol::app::online_game::display_walls()
{
  ZoneScoped;
...
void bim::axmol::app::online_game::display_fences(bim::game::cell_edge mask)
{
  ZoneScoped;

  bim::bit_map<bim::game::cell_edge, std::size_t, bim::game::cell_edge_count>
      asset_index{};

  for (const fence& f : m_fences)
    if (!!(f.edge & mask))
      {
        std::size_t& i = asset_index[f.edge];

        if (i == m_fence_assets[f.edge].size())
          continue;

        display_at(f.y, *m_fence_assets[f.edge][i],
                   m_display_config.grid_position_to_displayed_block_center(
                       f.x, f.y));
        ++i;
      }

  for (std::size_t i = 0; i != bim::game::cell_edge_count; ++i)
    {
      const bim::game::cell_edge e = (bim::game::cell_edge)(1 << i);

      if (!!(e & mask))
        bim::axmol::widget::hide_while_visible(m_fence_assets[e],
                                               asset_index[e]);
    }
}
...
void bim::axmol::app::online_game::display_animations()
{
  ZoneScoped;
  ...
  registry
      .view<bim::game::position_on_grid, bim::game::animation_state, Layer>()
      .each(
