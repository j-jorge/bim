...
std::jthread bm::app::console::launch_input_thread(std::atomic<int>& input)
{
  return std::jthread(
...
bool bm::app::console::apply_inputs(entt::registry& registry, int player_index,
                                    int input)
{
  if (input == 'q')
    return false;
  ...
  bm::game::player_action& player_action
      = [&registry, player_index]() -> bm::game::player_action&
  {
