...
// well as the level of indirections, we will use the PIMPL version that embeds
// its data, but then we must know how much space we need. The size of this
// space is defined by the implementation from implement_controls_struct.hpp:
...
#define bim_declare_controls_struct(type, member, count)                      \
  struct type;                                                                \
  iscool::memory::pimpl<type,                                                 \
                        count * sizeof(bim::axmol::ref_ptr<ax::Node>)         \
