...
namespace bim::game
{
  ...
  public:
    ...
    bot(const bot&) noexcept;
    bot(bot&&) noexcept;
    ~bot();

    bot& operator=(const bot&) noexcept;
  ...
  private:
    ...
    bool goal_is_feasible(const contest& contest, const player& bot_player,
    ...
    int sum_to(std::uint8_t x, std::uint8_t y,
    ...
    bool drop_bomb_and_find_safe_place(
        const contest& contest, const player& bot_player,
    ...
    [[gnu::cold]] void log_event(const char* name) const;
    [[gnu::cold]] void log_initial_state(const contest& contest) const;
    template <typename T>
    [[gnu::cold]] void log_player_array(const char* name,
                                        const per_player_array<T>& a) const;
    template <typename T>
    [[gnu::cold]] void log_map(const char* name,
                               const bim::table_2d<T>& m) const;

    [[gnu::cold]] void log_goal() const;
    [[gnu::cold]] void log_goal_summary(const goal& g, int reward) const;
  ...
  private:
    ...
    /** Cells where we die instantly if we walk on them. */
    bim::table_2d<bool> m_death_map;

    /**
     * Cells with a short-term danger (e.g bombs on the edge of exploding).
     * This includes m_death_danger_map.
     */
    bim::table_2d<bool> m_immediate_danger_map;

    /**
     * Long-term danger, e.g. area that is going to be covered by bombs. This
     * also includes m_immediate_danger_map.
     */
