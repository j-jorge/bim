      ...
      .each(
              ...
              -> void
          {
            if ((position.x == player_position.grid_aligned_x())
              ...
              registry.emplace_or_replace<bim::game::dead>(player_entity);
          });
...
void bim::game::update_falling_blocks(entt::registry& registry, arena& arena)
{
  registry.view<timer, falling_block, position_on_grid>().each(
          ...
          const position_on_grid& position) -> void
      {
        ...
        seal_falling_block(registry, arena, e, position);
      });
}
