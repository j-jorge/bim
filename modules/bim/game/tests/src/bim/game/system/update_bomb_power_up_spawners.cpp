...
TEST(update_bomb_power_up_spawners, burning)
{
  entt::registry registry;
  bim::game::arena arena(3, 3);

  const entt::entity entity = registry.create();
  registry.emplace<bim::game::bomb_power_up_spawner>(entity);
  registry.emplace<bim::game::burning>(entity);
  registry.emplace<bim::game::position_on_grid>(entity, 1, 2);

  bim::game::update_bomb_power_up_spawners(registry, arena);

  const entt::entity power_up = arena.entity_at(1, 2);
  ASSERT_TRUE(entt::null != power_up);
  EXPECT_TRUE(registry.storage<bim::game::bomb_power_up>().contains(power_up));
}
