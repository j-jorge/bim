  ...
  , m_last_completed_tick(0)
{
  ...
  update_exchange.checksum(m_last_confirmed_tick, m_last_confirmed_checksum);
...
void bim::net::contest_runner::queue_updates(const server_update& updates)
{
  ...
  assert(updates.from_tick == m_last_confirmed_tick + pre_update_tick_count);
  ...
  if (has_action)
    {
      ...
      if (m_last_confirmed_checksum != m_server_checksum)
        ic_log(iscool::log::nature::info(), "contest_runner",
               ...
               m_last_confirmed_tick, m_last_confirmed_checksum,
               m_server_checksum);

      m_update_exchange.checksum(m_last_confirmed_tick,
...
void bim::net::contest_runner::save_contest_state(entt::registry& registry)
{
...

  {
    ...
    m_last_confirmed_checksum = bim::crc32(m_last_confirmed_archive);
  }
