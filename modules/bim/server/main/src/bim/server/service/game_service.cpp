...
struct bim::server::game_service::game
{
  /*
    ...
    You know it's going to be bad when the comments contain ASCII art.
...
private:
  ...
  bool seal_player_actions(std::size_t count)
  {
    ...
    for (std::size_t i = 0; i != count; ++i)
      {
        bim::game::per_player_array<bim::game::player_action*> player_actions;
  ...
  void push_game_state_checksum()
  {
    const std::uint32_t checksum =
        bim::game::game_state_checksum(contest.registry());
    simulation_checksum.push_back(checksum);
  }
...
public:
  ...
  bim::game::per_player_array<bool> active;
    ...
    game_reward_availability reward_availability, bot_availability bot)
{
  ...
  bim::game::per_player_array<iscool::net::session_id> final_sessions(
      sessions);
  ...
  if (m_contest_timeline_service)
    game.timeline_writer = m_contest_timeline_service->open(
    ...
    std::chrono::nanoseconds now)
{
  ...
  if ((update->checksum_tick >= game.completed_tick_count_all)
      && (update->checksum
    ...
    const game& game)
{
  ...
  message.final_checksum =
      game.simulation_checksum[completed_tick_count_for_player + tick_count
                               - game.completed_tick_count_all];
    ...
    iscool::net::channel_id channel, game& game) const
{
  ...
  const bool has_a_winner = game.contest_result.has_a_winner();

  game.winner_index = has_a_winner ? game.contest_result.winning_player()
                                   : bim::game::g_max_player_count;

  const iscool::net::session_id winner_session =
      has_a_winner ? game.sessions[game.winner_index] : -1;
    ...
    std::chrono::nanoseconds now) const
{
  ...
  bim::game::per_player_array<int> player_tick;
  bim::game::per_player_array<std::uint8_t> player_index;
  int player_count = 0;
