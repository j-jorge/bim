...
#include <bm/game/archive_storage.hpp>
#include <bm/game/arena.hpp>
#include <bm/game/tick_counter.hpp>
...
namespace bm::net
{
  ...
  {
  public:
    contest_runner(bm::game::contest& contest,
  ...
  private:
    ...
    bm::game::contest& m_contest;
    game_update_exchange& m_update_exchange;

    bm::game::tick_counter m_tick_counter;

    std::vector<std::array<bm::game::player_action, 4>> m_server_actions;
    std::uint32_t m_last_confirmed_tick;
    std::uint32_t m_last_completed_tick;

    std::array<std::vector<bm::game::player_action>, 4> m_unconfirmed_actions;

    bm::game::archive_storage m_last_confirmed_archive;
    bm::game::arena m_last_confirmed_arena;
  };
}
