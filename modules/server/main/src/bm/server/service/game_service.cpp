...
struct bm::server::game_service::game
{
  ...
  */
  ...
  std::array<std::vector<bm::game::player_action>, 4> actions;
  ...
  void drop_old_actions()
  {
    ...
    bm_assume(player_count >= 2);
    bm_assume(player_count <= 4);
    ...
    const std::array<iscool::net::session_id, 4>& sessions)
{
  ...
  for (std::vector<bm::game::player_action>& v : game.actions)
    v.reserve(32);
                                       ...
                                       const iscool::net::message& message)
{
  ...
  switch (message.get_type())
    {
    case bm::net::message_type::ready:
      mark_as_ready(endpoint, message.get_session_id(), channel, it->second);
      break;
    case bm::net::message_type::game_update_from_client:
      push_update(endpoint, message.get_session_id(), channel,
                  bm::net::game_update_from_client(message.get_content()),
                  it->second);
      break;
    }
}
...
void bm::server::game_service::push_update(
    const iscool::net::endpoint& endpoint, iscool::net::session_id session,
    iscool::net::channel_id channel,
    const bm::net::game_update_from_client& message, game& game)
{
...
std::optional<std::size_t> bm::server::game_service::validate_message(
    const bm::net::game_update_from_client& message, std::size_t player_index,
    const game& game) const
{
  ...
  if (message.actions.size() > bm::game::player_action::queue_capacity * 255)
    return std::nullopt;
...
void bm::server::game_service::queue_actions(
    const bm::net::game_update_from_client& message, std::size_t player_index,
    game& game)
{
  std::size_t tick_index = game.completed_tick_count_all
                           ...
                           - message.from_tick;
  bm_assume(tick_index <= message.action_count_at_tick.size());
  ...
  for (; tick_index != tick_count; ++tick_index)
    {
      ...
      bm_assume(action_count <= bm::game::player_action::queue_capacity);
...
void bm::server::game_service::send_actions(
    const iscool::net::endpoint& endpoint, iscool::net::session_id session,
      ...
      for (std::uint8_t player = 0; player != game.player_count; ++player)
        {
          ...
          bm_assume(action_index <= game.actions[player].size());
