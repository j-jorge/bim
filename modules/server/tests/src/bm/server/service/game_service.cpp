...
TEST(game_service, new_game)
{
  ...
  bm::server::game_service service(socket_stream);

  const bm::server::game_info game = service.new_game(4, { 11, 22, 33, 44 });
  ...
  const std::optional<bm::server::game_info> game_opt
